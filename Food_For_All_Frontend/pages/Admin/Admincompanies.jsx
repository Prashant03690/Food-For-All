...
export default function Admincompanies() {
  ...
  const loadCompanies = async () => {
    const result = await axios.get("http://localhost:8080/getAllCompanies");
    setCompany(result.data);
  };

  const deleteProducts = async (coid) => {
    const result = await axios.delete(`http://localhost:8080/deleteCompanies/${coid}`);
    loadCompanies();
  };
              ...
              <tbody>
                {
                  ...
                  (
                    <tr>
                      ...
                      <td>{company.cophone}</td>
                      <td>{company.city.cityname}</td>
                      <td> <button className="btn btn-danger mx-2" onClick={() => deleteProducts(company.coid)}>Delete</button></td>
