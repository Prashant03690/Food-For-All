...
export default function AdminLogin() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:8080/checkAdminLogin", {
        email,
        password
      })
      .then((response) => {
        if (response.data) {
          ...
          navigate(`/adminpage/${response.data.aid}/${response.data.fname}/${response.data.lname}/${response.data.aemail}/${response.data.aphone}`);
        }
        else {
